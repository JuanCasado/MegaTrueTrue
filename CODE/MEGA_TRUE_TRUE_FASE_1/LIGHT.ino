...
#define T_CIRCULO_E 30
  ...
  if ((millis() - tiempo_anterior) > T_CIRCULO_G) {
    tiempo_anterior = millis();
    for (int i = 0; i < NUMPIXELS; i++) {
      pixels.setPixelColor(i, pixels.Color(color, 0, 255));
    }
    pixels.show();
  }
  if (color == 255)
...
#define T_CIRCULO_P 20
  ...
  if ((millis() - tiempo_anterior) > T_CIRCULO_E) {
    tiempo_anterior = millis();
    pixels.setPixelColor(i, BLANCO_);
    i ++;
    if (i >= NUMPIXELS) {
      i = 0;
    }
...
#define T_CIRCULO_L 20
  ...
  if ((millis() - tiempo_anterior) > T_CIRCULO_E) {
    tiempo_anterior = millis();
    pixels.setPixelColor(a, MORADO_);
    pixels.setPixelColor(b, MORADO_);
    pixels.setPixelColor(c, MORADO_);
    pixels.setPixelColor(d, MORADO_);
    a ++;
    b ++;
    c ++;
    d ++;
